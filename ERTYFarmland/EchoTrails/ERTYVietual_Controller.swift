...
class ERTYVietual_Controller: EDCLEARHIkingMainBasci, CenterGHeadeDelegate{
    var headerMeView:ERTYCenterTopHeader?
    
    func bugRepellent(buttontag: Int) {
        var totlaLinder = ERTYBUIEDARTrailRequestScout.ERTYBUIEDpathfinder.vistaWebUrl
        
        switch buttontag {
        case 10:
            totlaLinder  = totlaLinder + "Dx4YGgxQLBoLCxYRGFAWERsaB0A".ERTYBUIEDhikeReflections()
            
        case 20:
            totlaLinder = totlaLinder + "Dx4YGgxQOhsWCzseCx5QFhEbGgdA".ERTYBUIEDhikeReflections()
        case 30:
            totlaLinder = totlaLinder + "Dx4YGgxQKRAKHBcaDTwaEQsaDVAWERsaB0A".ERTYBUIEDhikeReflections()
        case 40:
            totlaLinder = totlaLinder + "Dx4YGgxQPA0aHgsaLRATGlAWERsaB0A".ERTYBUIEDhikeReflections()
        default:
            break
        }
...
extension ERTYVietual_Controller:UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let emptyImage = UIImageView.init(image: UIImage.init(named: "ERTYBUIEDnotinhRecird"))
    ...
    @objc func requestForDymAllHikeData()  {
        guard let useID = ERTYBUIEDARTrailRequestScout.ERTYBUIEDpathfinder.wildernessGuide?["quickDryShirt"] as? Int else{
            return
        }
        
        ERTYBUIEDARTrailRequestScout.ERTYBUIEDpathfinder.exploreWilderness(destination: "/zatsqmegbrjz/adoexqmwl",provisions:["basecampLife":"\(useID)"],needsGuide:true) { dataResult in
             
             guard let hikebackdata = dataResult as? Dictionary<String,Any> ,

                   let hikedata = hikebackdata["Gx4LHg".ERTYBUIEDhikeReflections()] as? Dictionary<String,Any>
